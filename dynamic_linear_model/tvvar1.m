function [l] = tvvar1(Y,Ynames,k,F,delta,beta,n0,r,arp)
%%%%%%%%%%%%
...
%std(Y(:,1:k)') = 1x10 vector; %std(Y(:,1:k)')' = 10x1; 
%vy = 10x1 or qx1
...
z = zeros(p,q); zq=zeros(q,1); M0=z; 
        if (p>1)
              ...
              %I = eye( sz ) returns an array with ones on the main diagonal and zeros elsewhere. 
              %The size vector, sz , defines size(I) . For example, eye([2,3]) returns a 2-by-3 ...
...
Mt = M0; C0=eye(p); Ct=C0;       %C0 is pxp; Ct=C0;        % initial Theta prior 
...
% forward filtering: 
        for t = k+1:T
            ...
            %Mt is pxq matrix that stores the estimates of coefficients
